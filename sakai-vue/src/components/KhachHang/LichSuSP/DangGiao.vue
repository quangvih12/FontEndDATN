...
<template>
    ...
    <div v-for="(items, index) in listSP" :key="index">
        ...
        <div class="san-pham">
            <p>
                <span class="hoan-thanh">{{ items.trangThai == 4 ? 'Đang giao' : 'Giao thất bại' }} <i class="pi pi-times" style="font-size: 1rem"></i></span> <span class="gach"> | </span>
                <span class="giao-thanh-cong"> {{ items.trangThai == 4 ? 'Đơn hàng đang được giao' : 'Đơn hàng đã được giao thất bại' }} <i class="pi pi-truck" style="font-size: 1.5rem"></i></span>
            </p>
        </div>
