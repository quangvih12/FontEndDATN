...
const onSubmit = handleSubmit(async (values) => {
    ...
    let sum = 0;
    for (const mauSac of props.myProp) {
    ...
    if (isDuplicate) {
        ...
        toast.add({ severity: 'error', summary: 'Error', detail: 'Màu sắc đã tồn tại', life: 3000 });
    } else if (sum + values.soLuong > props.soLuongTong) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Tổng số lượng màu sắc lớn hơn số lượng tổng ' + sum + values.soLuong + ' > ' + props.soLuongTong, life: 3000 });
    } else {
        await productStore.addMauSac(form);
...
const openNew = () => {
    ...
    soLuong.value = props.soLuongTong - sum;
    soLuongs.value = props.soLuongTong - sum;
                    ...
                    <div class="Field col-12 md:col-12" style="margin-bottom: 30px">
                        <div style="width: 1000px; text-align: center;display: flex; margin-left: 120px;">
                            ...
                            <Button type="submit" class="p-button-outlined" style="width: 100px; height: auto; margin: 10px"
                                label="Lưu" v-if="soLuongs > 0"></Button>
                        </div>
                    </div>
