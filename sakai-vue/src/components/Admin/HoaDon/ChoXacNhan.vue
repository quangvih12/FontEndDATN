...
const searchDate = async () => {
    ...
    if (phuongThucThanhToan.value == 'tatCa' || phuongThucThanhToan.value == null) {
        ...
        if (startDate.value == null || startDate.value.length <= 0 || endDate.value == null || endDate.value.length <= 0) {
            ...
            const respone = await useHD.searchDateByTrangThaiAndPttt(startDate.value, endDate.value, 'ngayTao', 2, parseInt(phuongThucThanhToan.value.value));
            data.value = respone;
        } else {
            const respone = await useHD.searchDateByTrangThaiAndPttt(startDate.value, endDate.value, typeSearchDate.value.value, 2, parseInt(phuongThucThanhToan.value.value));
            data.value = respone;
        }
    }
};
