...
const schema = yup.object().shape({
    ...
    anh: yup.string()
        .required('vui lòng chọn ảnh chính cho sản phẩm'),
});
...
const deleteMauSac = async (idMauSac) => {
    try {

        const mauSacToDelete = selectedMauSac.value.find(m => m.id === idMauSac);
        ...
        const updatedSelectedMauSac = selectedMauSac.value.filter(m => m.id !== idMauSac);
        ...
        props.myProp.mauSac = props.myProp.mauSac.filter(s => s.mauSac.id !== idMauSac);

        // Gọi hàm xóa từ store nếu cần
        await productStore.deleteMauSac(props.myProp.id, idMauSac);
    } catch (error) {
