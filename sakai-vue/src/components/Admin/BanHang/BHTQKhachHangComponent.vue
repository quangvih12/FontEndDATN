...
const onRowEditSave = async (event) => {
  ...
  if (event.data.id === null) {
    ...
    if (event.data.id !== null) { toast.add({ severity: 'info', summary: 'Thông báo', detail: 'Không có thay đổi', life: 3000 }); return; }
    else { store.dsKH.shift(); return; }
  } else {
    try {
      await store.addOrUpdateKH(event.data);
      if (event.data.id === null) toast.add({
    ...
    <template #header>
      <div class="flex justify-content-between align-items-center">
        ...
        <span class="p-input-icon-left">
          ...
          <InputText :disabled="tableKHEditingRows.length > 0" placeholder="Tìm kiếm khách hàng..." />
        </span>
