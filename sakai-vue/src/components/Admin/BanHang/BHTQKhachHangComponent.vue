...
import {deepEqual} from "@/service/common/ObjectUtils";
...
const onRowEditSave = async (event) => {
  ...
  if (JSON.stringify(event.data) === JSON.stringify(event.newData)) {
    if (event.data.id !== null) toast.add({severity: 'info', summary: 'Thông báo', detail: 'Không có thay đổi', life: 3000});
    else store.dsKH.shift();
  } else {
    try {
      ...
      else toast.add({severity: 'success', summary: 'Thành công', detail: 'Hoàn tất cập nhật thông tin KH', life: 3000});
    } catch (err) {
...
<template>
  <DataTable :value="dsKH"
             dataKey="id"
             editMode="row"
             v-model:selection="selectedKhachHang"
             selectionMode="single"
             v-model:editingRows="tableKHEditingRows"
      ...
      <div class="flex justify-content-between align-items-center">
        ...
        <span class="p-input-icon-left">
                                    <i class="pi pi-search"/>
                                                <InputText :disabled="tableKHEditingRows.length > 0"
