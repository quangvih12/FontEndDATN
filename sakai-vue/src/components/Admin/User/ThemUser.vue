...
<template>
    ...
    <!-- <div class="grid">
        <div class="col-12">
            <div class="card"> -->
    <Dialog v-model:visible="productDialog" :style="{ width: '600px' }" header="Thêm user" :modal="true" class="p-fluid">
        <div class="flex">
            <!-- Cột phải -->
            <div class="p-col-6">
                <div class="field">
                    ...
                    <small class="p-invalid" v-if="submitted && !product.inventoryStatus">Vui lòng chọn chức vụ</small>
                </div>
                <div class="field">
                    <label for="name">Tên người dùng</label>
                    <InputText id="name" v-model.trim="product.name" required="true" autofocus :class="{ 'p-invalid': submitted && !product.name }" />
                    <small class="p-invalid" v-if="submitted && !product.name">Tên không được trống</small>
                </div>
                <div class="field">
                    <label for="name">Mật khẩu</label>
                    <Password v-model="product.pass" toggleMask />
                    <small class="p-invalid" v-if="submitted && !product.pass">Mật khẩu không được trống</small>
                </div>
                <div class="field">
                    ...
                    <small class="p-invalid" v-if="submitted && !product.category">Vui lòng chọn giới tính</small>
                </div>

                <div class="field">
                    <label for="name">Ngày sinh</label>
                    <div><InputText type="date" class="input" v-model.trim="product.ngaySinh" /></div>
                    <small class="p-invalid" v-if="submitted && !product.ngaySinh">Ngày sinh không được trống</small>
                ...
                <div class="field">
                    ...
                    <small class="p-invalid" v-if="submitted && !product.sdt">Số điện thoại không được trống</small>
                </div>
                <div class="field">
                    <label for="name">Email</label>
                    <InputText id="name" v-model.trim="product.email" required="true" autofocus :class="{ 'p-invalid': submitted && !product.name }" />
                    <small class="p-invalid" v-if="submitted && !product.email">Email không được trống</small>
                </div>
                <div class="field">
                    <label for="adress" class="mb-3">Địa chỉ </label>
                    <div class="flex">
                        <Dropdown id="adress" v-model="product.adress" :options="valueChucVu" optionLabel="label" placeholder="Select a Status" style="width: 90%; margin-right: 20px">
                            <template #value="slotProps">
                                <div v-if="slotProps.value && slotProps.value.value">
                                    <span :class="'product-badge status-' + slotProps.value.value">{{ slotProps.value.label }}</span>
                                </div>
                    ...
                    <small class="p-invalid" v-if="submitted && !product.adress">Địa chỉ không được trống</small>
                </div>
            </div>
            <!-- cột trái -->
